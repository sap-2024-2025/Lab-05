...
class PixelGridProxy implements PixelGridAsyncAPI {

    static Logger logger = Logger.getLogger("[PixelArt Service Proxy]");	
	private HttpClient client;
	private Vertx vertx;
	private URL gridAddress;
	
	public PixelGridProxy(URL gridAddress) {
		vertx = Vertx.vertx();
		this.gridAddress = gridAddress;
		HttpClientOptions options = new HttpClientOptions()
					.setDefaultHost(gridAddress.getHost())
					.setDefaultPort(gridAddress.getPort());
	...
	public Future<JsonObject> subscribePixelGridEvents(PixelGridEventObserver l) {
		...
		WebSocketConnectOptions wsoptions = new WebSocketConnectOptions()
				  .setHost(gridAddress.getHost())
				  .setPort(gridAddress.getPort())
