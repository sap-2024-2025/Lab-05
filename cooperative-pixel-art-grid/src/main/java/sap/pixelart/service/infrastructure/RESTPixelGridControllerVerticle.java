 ...
 *
 */
public class RESTPixelGridControllerVerticle extends AbstractVerticle implements PixelGridEventObserver {

	private int port;
	private PixelGridAPI pixelArtAPI;
	static Logger logger = Logger.getLogger("[PixelArt Service]");
	static String PIXEL_GRID_CHANNEL = "pixel-grid-events";

	public RESTPixelGridControllerVerticle(int port, PixelGridAPI appAPI) {
		this.port = port;
	...
	public void start() {
		logger.log(Level.INFO, "Pixel Grid initializing...");
		...
		logger.log(Level.INFO, "PixelArt Grid ready - port: " + port);
	}
